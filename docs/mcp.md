...
it creates elements, connects ports, validates the document and can render a PNG
...
4. Connect them with cold-water pipes in a left-to-right chain.
5. Validate the diagram.
6. Render a PNG preview to exports/agent-preview.png.
7. Tell me the validation result and where the PNG was written.
```
...
pipes. Validate it and render a PNG preview.
```
...
validate the diagram and render a PNG.
```
...
Ask it to run `hydraulic_list_catalog` first. Element types and port ids must
come from the catalog, not from guessed human labels.
