...
- Diagram autosave lives behind `src/features/diagram-editor/persistence/diagramAutosave.ts`, uses the same `DiagramDocument` format, and must validate before restore.
...
- Autosave restores durable diagram data without pushing an undo/redo history checkpoint.
